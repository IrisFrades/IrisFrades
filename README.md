# 👋 Buenas, soy Iris

💻 Estudiante de programación y videojuegos
🚀 Me encantan la creación de videojuegos y aprender cosas nuevas

---

## 🖥️ Acceso a datos
- 🔗 [Inventario](https://github.com/IrisFrades/Inventory_FradesIris)  
  Sistema de inventario con bases de datos SQLite

- 🔗 [Login](https://github.com/IrisFrades/Login_FradesIris)  
  Login + Register en Unity
  
- 🔗 [Kahoot](https://github.com/IrisFrades/IrisFrades_Kahoot)  
  App para hacer kahoots y jugarlos
  
- 🔗 [Login](https://github.com/IrisFrades/UF1_ACT1_IrisFrades.git)  
  Login + Register en Unity (con ORMs)
---

## 🌐 PSP + SPG

---

## 🤖 Desarrollo interfícies
- 🔗 [Vuforia](https://github.com/IrisFrades/FradesIris_Vuforia)  
  Taller de vuforia para ver personajes a traves del movil con QR

- 🔗 [Menu pausa](https://github.com/IrisFrades/Disseny_Interf-cies_RA1_ACT2_PP-)  
  Actividad mini menu de pausa

- 🔗 [Canvas, Botones y Event System](https://github.com/IrisFrades/UF1_ACT1_IrisFrades)  
  Escena donde se simulará el feedback por GUI de la salud de un personaje de un juego (quitar vida, aumentar vida, matarlo, revivirlo)

- 🔗 [UI Mask + Render Texture + Tipografia](https://github.com/IrisFrades/FradesIris_UIMask_RenderTexture_Tipografia.git)  
  Actividad de UI Mask + Render Texture + Tipografia en Unity
  
---

## 🤖 Desarrollo móvil
- 🔗 [Minijuego arcade](https://github.com/IrisFrades/FradesIris_MiniJocArcadeMobile)  
  Juego arcade de bob esponja hecho en Unity
  
- 🔗 [Dotween](https://github.com/IrisFrades/DOTween_FradesIris)  
  Actividad de dotween (movimiento de objetos)

- 🔗 [FSM1](https://github.com/IrisFrades/FSM_IrisFrades)  
  Actividad de FSM movimiento puerta (sencillo)

- 🔗 [FSM2](https://github.com/IrisFrades/FSM_ActivitatPorta_FradesIris)  
  Actividad de FSM movimiento puerta (complicado)

- 🔗 [Android Studio](https://github.com/IrisFrades/Login_Register_FradesIris)  
  Cambio de pantallas con actividades dentro

---

## 🤖 IPO
- 🔗 [Ruleta](https://github.com/IrisFrades/Projecte-Ruleta-IPO)  
  Primer proyecto de unity de IPO

